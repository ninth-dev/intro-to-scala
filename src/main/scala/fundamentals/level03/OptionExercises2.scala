...
object OptionExercises2 {
  ...
  def findJobDescriptionGivenJobId1(jobId: JobId): Option[String] =
    findJobById(jobId) match {
      case Some(job: Job) => Some(job.description)
      case _ => None
    }
  ...
  def findJobDescriptionGivenJobIdOrElse2(jobId: JobId): String =
    findJobDescriptionGivenJobId2(jobId).getOrElse(s"Job with id ${jobId} does not exist")
  /**
  ...
  /**
    ...
    */
//  def findJobByHumanId(humanId: HumanId): Option[Job] = findJobIdByHumanId(humanId).map(id => findJobById(id)).flatten
