...
/**
  * These exercises introduce data types and also algebraic data types (ADTs).
  * ADTs are a huge part of typed functional programming.
...
object TypesExercises {
  ...
  def purchase(cost: Double, wallet: Wallet): Wallet = {
    val amountRemaining = wallet.amount - cost
    wallet.copy(amount = amountRemaining)
  }
  ...
  def showTrafficLightStr(trafficLight: String): String =
    trafficLight match {
      ...
      case _ => "The traffic light is red"
    }
